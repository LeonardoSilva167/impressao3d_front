...
import { buscarMesPorNumero, formatarParaMoedaSemSimbolo, mesesSelect, numberFormat, useNavegacao } from 'helpers/functions_helpers';
...
export interface VendasTableProps {
    data: PaginateInterface<VendasList> | undefined,
    getData: (data: PaginateSearch & VendasSearch) => void,
...
export const VendasTable = ({ data, getData, setPerPage, setPage, perPage, filters }: VendasTableProps) => {
    ...
    const handleRemoteDelete = async (id: number) => {
        try {
            const response = await vendasService.deleteVendas(id);
            if (data) {
    ...
    const handleThisRoute = async (url: string) => {
        try {
            ...
            await getData({
                ...
                cliente_id: new_url.searchParams.get('Vendas_id'),
                                                                ...
                                                                {data && data.data.map((row: any, index: number) => {
                                                                    return (    
                                                                        <tr>
                                                                            ...
                                                                            <td>{`${row.data_venda_format}`}</td>
                                                                            <td className="text-end">{formatarParaMoedaSemSimbolo(row.total)}</td>
                                                                            {/* <td><span className={`text-center badge bg-${row.finalizado ? 'dark' : 'primary'}`}>{`${row.finalizado ? 'Finalizada' : 'Aberta'}`}</span></td>                                                                         */}
                                                                            <td>
                                                                                <ButtonGroup>
                                                                                    <UncontrolledDropdown direction="down">
                                                                                        ...
                                                                                        <DropdownMenu style={{ zIndex: '999' }}>
                                                                                            <Link to={`/vendas/edit/${row.id}`} state={{ source: row }}>
