...
import { FluxoCaixaList, FluxoCaixaSearch } from "interfaces/FluxoCaixa";
...
export interface FluxoCaixaTableProps {
    data: PaginateInterface<FluxoCaixaList> | undefined,
    getData: (data: PaginateSearch & FluxoCaixaSearch) => void,
...
export const FluxoCaixaTable = ({ data, getData, setPerPage, setPage, perPage, filters }: FluxoCaixaTableProps) => {
                                                ...
                                                <div className="d-flex flex-row justify-content-end align-items-center mb-4" >
                                                    <div className="mt-3 mt-lg-0">
                                                        <div className="col-auto">
                                                            <Link to={`/fluxo-caixa/caixa/2025-07-01/2025-07-07`} type="button" className="btn btn-primary"><i className="ri-currency-fill me-1 align-middle"></i> Fluxo de Caixa</Link>
                                                        </div>
                                                    </div>
                                                </div>
                                            </Col>
                                                    ...
                                                    <table className="table align-middle table-nowrap table-striped-columns mb-0 text-center">
                                                        <thead className="table-light">
                                                            <tr>
                                                                ...
                                                                <th className="text-end" scope="col">Entradas</th>
                                                                <th className="text-end" scope="col">Saida</th>
                                                        ...
                                                        <tbody>
                                                            {data && data.data.map((row: any, index: number) => {
                                                                ...
                                                                return (    
                                                                    <tr>
                                                                        <td className="text-start">{row.data_movimentacao_format}</td>
                                                                        <td className="text-end">{formatarParaMoedaSemSimbolo(row.total_entradas)}</td>
                                                                        <td className="text-end">{formatarParaMoedaSemSimbolo(row.total_saidas)}</td>
                                                                        <td className="text-end">{formatarParaMoedaSemSimbolo(row.total_movimentos)}</td>
                                                                        <td>
                                                                            <ButtonGroup>
                                                                                <UncontrolledDropdown direction="down">
                                                                                    ...
                                                                                    <DropdownMenu style={{ zIndex: '999' }}>
                                                                                        <Link to={`/fluxo-caixa/caixa/${row.data}`} state={{ source: row }}>
                                                                                            <DropdownItem>Fluxo de Caixa</DropdownItem>
                                                                                        </Link>
                                                                                    </DropdownMenu>
