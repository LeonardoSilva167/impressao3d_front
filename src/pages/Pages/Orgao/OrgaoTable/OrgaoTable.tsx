...
export interface OrgaoTableProps {
    data: PaginateInterface<OrgaoList> | undefined,
    getData: (data: PaginateSearch & OrgaoSearch) => void,
...
export const OrgaoTable = ({ data, getData, setPerPage, setPage, perPage, filters }: OrgaoTableProps) => {
    ...
    const handleRemoteDelete = async (id: number) => {
        try {
            const response = await orgaoService.deleteOrgao(id);
            if (data) {
    ...
    const handleThisRoute = async (url: string) => {
        try {
            ...
            await getData({
                ...
                cliente_id: new_url.searchParams.get('Orgao_id'),
                                                    ...
                                                    <div className="table-responsive">
                                                        <table className="table align-middle table-nowrap table-striped-columns mb-0 text-center">
                                                            <thead className="table-light">
                                                                <tr>
                                                                    <th scope="col" className="text-start">Orgão</th>
                                                                    <th scope="col">Código</th>
                                                                    <th scope="col" className="text-start">Unidade Compradora</th>
                                                                    <th scope="col">UF</th>
                                                            ...
                                                            <tbody>
                                                                {data && data.data.map((row: any, index: number) => {
                                                                    ...
                                                                    return (    
                                                                        <tr>
                                                                            <td className="text-start">{row.orgao_nome}</td>
                                                                            <td className="text-right">{`${row.codigo}`}</td>
                                                                            <td className="text-start">{row.nome}</td>
                                                                            <td className="text-center">{row.uf}</td>
                                                                            <td className="text-center">{row.cidade}</td>
                                                                            <td>
                                                                                <ButtonGroup>
                                                                                    <UncontrolledDropdown direction="down">
                                                                                        ...
                                                                                        <DropdownMenu style={{ zIndex: '999' }}>
                                                                                            <Link to={`/orgao-participante/edit/${row.orgao_id}`} state={{ source: row }}>
                                                                                                <DropdownItem>Editar</DropdownItem>
                                                                                            </Link>
                                                                                            <DropdownItem
                                                                                                onClick={() => {
                                                                                                    setSelectedId(row.orgao_id);
                                                                                                    toggleModal();
                                                                                                }}
                                                                                            >
                                                                                                Excluir
