...
import { OrgaoModel, OrgaoSearch } from "interfaces/Orgao/OrgaoInterface";


export interface OrgaoFilterProps {
    getRemoteOrgaoList: (data: any) => void
}

const OrgaoFilter = ({ getRemoteOrgaoList }: OrgaoFilterProps) => {
    const orgaoService = new OrgaoService()

    const { handleSubmit, control, setValue, register, watch } = useForm<OrgaoSearch>({ defaultValues: {} })
    ...
    const getListOrgao = async (inputValue: string): Promise<SelectOptions[]> => {
        const listOrgao = await orgaoService.AsyncListOrgao({ palavra_chave: inputValue })
        let opt = new Array
        opt.push({ value: '', label: 'Selecione' })

        listOrgao && listOrgao.map((item: OrgaoModel) => {
                ...
                <Col xs={12}>
                    <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                        <div className='d-sm-flex align-items-center justify-content-between'>
                            ...
                            <h4 className="mb-sm-0 ms-3">Orgao</h4>
                        </div>
                        <Breadcrumb pageTitle="" listClassName='mb-sm-0 pt-1 py-2'>
                            <BreadcrumbItem> <Link to="/dashboard"> <i className="ri-home-5-fill"></i> </Link> </BreadcrumbItem>
                            <BreadcrumbItem active> Orgao </BreadcrumbItem>
                        </Breadcrumb>
                    </div>
                </Col>
            </Row>
                        ...
                        <CardHeader>
                            <div className="gap-2 flex-wrap">
                                <Row>
                                    ...
                                    {!showFilter &&
                                        <Col md={8}>
                                            <form onSubmit={handleSubmit(getRemoteOrgaoList)} >
                            ...
                            <Row>
                                <Col>
                                    <Collapse isOpen={showFilter} id="multiCollapseExample2" className="multi-collapse mt-3">
                                        <div className="">
                                            <form className="px-0 my-0 m-2" id="form-search" name="form-search" onSubmit={handleSubmit(getRemoteOrgaoList)}>
                                                <Row>
                                                    <Col md={5}>
                                                        <Label htmlFor="cliente_id" className="form-label">Orgao</Label>
                                                        <AsyncSelectListControlled<OrgaoSearch>
                                                            callback={getListOrgao}
                                                            field={"cliente_id"}
                                                            control={control}
                                                            className="w-100"
                                                        />
                                                    ...
                                                    <Col md={2}>
                                                        <div className="mb-3">
                                                            ...
                                                            <SelectListControlled<OrgaoSearch> 
                                                                options={optUF} 
                                                                field={"uf"} 
                                                                control={control} 
                                                            />
                                                    ...
                                                    <Col md={2}>
                                                        <div className="ms-4 ">
                                                            ...
                                                            <InputTextControlled<OrgaoSearch> 
                                                                field={"cidade"} 
                                                                control={control} 
                                                            />
                                                        </div>                                     
                                                ...
                                                <Row className="mt-5">
                                                    <div className="d-flex flex-row justify-content-end align-items-center" >
                                                        ...
                                                        <Col md={6}>
                                                            <div className="ms-4 ">
                                                                <InputTextControlled<OrgaoSearch> field={"palavra_chave"} control={control} placeholder="Buscar..." />
                                                            </div>
                                                        </Col>
