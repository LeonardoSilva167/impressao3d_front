...
export interface SubtipoProdutoTableProps {
    data: PaginateInterface<SubtipoProdutoList> | undefined,
    getData: (data: PaginateSearch & SubtipoProdutoSearch) => void,
...
export const SubtipoProdutoTable = ({ data, getData, setPerPage, setPage, perPage, filters }: SubtipoProdutoTableProps) => {
    ...
    const handleRemoteDelete = async (id: number) => {
        try {
            const response = await subtipoProdutoService.deleteSubtipoProduto(id);
            if (data) {
    ...
    const handleThisRoute = async (url: string) => {
        try {
            ...
            await getData({
                tipo_produtos_id: new_url.searchParams.get('tipo_produtos_id'),
                                                                ...
                                                                {data && data.data.map((row: any, index: number) => {
                                                                    ...
                                                                    return (    
                                                                        <tr>
                                                                            ...
                                                                            <td className=" text-start">{row.tipo_produtos_nome}</td>
                                                                                ...
                                                                                <ButtonGroup>
                                                                                    <UncontrolledDropdown direction="down">
                                                                                        ...
                                                                                        <DropdownMenu style={{ zIndex: '999' }}>
                                                                                            <Link to={`/tipo-produto/edit/${row.id}`} state={{ source: row }}>
...
export default SubtipoProdutoTable;
