...
export interface LancamentosTableProps {
    data: PaginateInterface<LancamentosList> | undefined,
    getData: (data: PaginateSearch & LancamentosSearch) => void,
...
export const LancamentosTable = ({ data, getData, setPerPage, setPage, perPage, filters }: LancamentosTableProps) => {
    ...
    const handleRemoteDelete = async (id: number) => {
        try {
            const response = await lancamentosService.deleteLancamentos(id);
            if (data) {
    ...
    const handleThisRoute = async (url: string) => {
        try {
            ...
            await getData({
                ...
                lancamento_id: new_url.searchParams.get('lancamento_id'),
                tipo_lancamento: Boolean(new_url.searchParams.get('tipo_lancamento')),
                despesa_id: new_url.searchParams.get('despesa_id'),
                receita_id: new_url.searchParams.get('receita_id'),
                dthr_lancamento: new_url.searchParams.get('dthr_lancamento'),
                valor: new_url.searchParams.get('valor'),
                contas_bancaria_origem_id: new_url.searchParams.get('contas_bancaria_origem_id'),
                contas_bancaria_destino_id: new_url.searchParams.get('contas_bancaria_destino_id'),
                                                    ...
                                                    <div className="table-responsive">
                                                        <table className="table align-middle table-nowrap table-striped-columns mb-0 text-center">
                                                            <thead className="table-light">
                                                                <tr>
                                                                    <th scope="col">ID Lancamento</th>
                                                                    <th scope="col">Tipo</th>
                                                                    <th scope="col">Receita/Despesa</th>
                                                            ...
                                                            <tbody>
                                                                {data && data.data.map((row: any, index: number) => {
                                                                    ...
                                                                    return (    
                                                                        <tr>
                                                                            <td>{row.lancamento_id}</td>
                                                                            <td><span className={`badge bg-${row.tipo_lancamento ? 'danger' : 'success'}`}>{`${row.tipo_lancamento ? 'Despesa' : 'Receita'}`}</span></td>
                                                                            <td>{`${row.tipo_lancamento ? row.nome_despesa : row.nome_receita}`}</td>
                                                                            <td>{row.dthr_lancamento_format}</td>
                                                                            <td>{numberFormat(parseFloat(row.valor), 2, ',', '.')}</td>
                                                                            <td>{row.banco_origem}</td>
                                                                                ...
                                                                                <ButtonGroup>
                                                                                    <UncontrolledDropdown direction="down">
                                                                                        ...
                                                                                        <DropdownMenu style={{ zIndex: '999' }}>
                                                                                            <Link to={`/lancamentos/edit/${row.id}`} state={{ source: row }}>
