...
export interface LicitacoesItensTableProps {
    data: PaginateInterface<LicitacoesItensList> | undefined,
    getData: (data: PaginateSearch & LicitacoesItensSearch) => void,
...
export const LicitacoesItensTable = ({ data, getData, setPerPage, setPage, perPage, filters }: LicitacoesItensTableProps) => {
    ...
    const handleRemoteDelete = async (id: number) => {
        try {
            const response = await licitacoesService.deleteLicitacoesItens(id);
            if (data) {
                                                                ...
                                                                {data && data.data.map((row: any, index: number) => {
                                                                    ...
                                                                    return (    
                                                                        <tr className={`${colorRowLicitacao(row.data_limite_proposta_format_date)}`}>
                                                                            ...
                                                                            <td className="text-start">{`${row.cliente_codigo} - ${row.cliente_nome}`}</td>
                                                                            <td>{row.data_limite_proposta_format}</td>
                                                                            <td><span className={`badge fs-12 bg-${colorStatusLicitacao(row.status_licitacoesItens_id)}`}>{`${row.status_licitacao}`}</span></td>
...
export default LicitacoesItensTable;
