...
export const ProdutosTable = ({ data, getData, setPerPage, setPage, perPage, filters }: ProdutosTableProps) => {
    ...
    const handleRemoteDelete = async (id: number) => {
        try {
            const response = await produtosService.deleteProdutos(id);
            if (data) {
                                                        ...
                                                        <table className="table align-middle table-nowrap table-striped-columns mb-0 text-center">
                                                            ...
                                                            <tbody>
                                                                {data && data.data.map((row: any, index: number) => {
                                                                    ...
                                                                    return (    
                                                                        <tr>
                                                                            <td className="text-left">{`${row.codigo_base}`}</td>
                                                                            <td>{row.descricao}</td>
                                                                            <td>
                                                                                <ButtonGroup>
                                                                                    <UncontrolledDropdown direction="down">
                                                                                        ...
                                                                                        <DropdownMenu style={{ zIndex: '999' }}>
                                                                                            <Link to={`/produtos/edit/${row.codigo_base}`} state={{ source: row }}>
