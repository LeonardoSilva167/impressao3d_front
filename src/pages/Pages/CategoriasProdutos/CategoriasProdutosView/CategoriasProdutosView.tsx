...
import { CategoriasProdutosView } from 'interfaces/CategoriasProdutos/CategoriasProdutosInterface'
import { CategoriasProdutosService } from 'services/CategoriasProdutos/CategoriasProdutosService'

const CategoriasProdutosViewPage = () => {
    const { id } = useParams()
    const { voltarParaRotaAnterior } = useNavegacao()
    const categoriasProdutosService = new CategoriasProdutosService()

    const [registro, setRegistro] = useState<CategoriasProdutosView>()
    ...
    const loadRegistro = async () => {
        ...
        setLoading(true)
        try {
            const view = await categoriasProdutosService.getViewCategoriasProdutos({ id: registroId })
            if (view) setRegistro(view)
        } catch (error) {
            console.error('Erro ao carregar categoria:', error)
            toast.error('Erro ao carregar categoria de produto.')
        } finally {
            setLoading(false)
        }
    }
                        ...
                        <Col xs={12}>
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                                <div className="d-sm-flex align-items-center justify-content-between">
                                    ...
                                    <h4 className="mb-sm-0 ms-3">Visualizar Categoria de Produto</h4>
                                </div>
                            ...
                            <Card>
                                <CardBody>
                                    {loading ? (
                                        ...
                                        <div className="text-center py-5 text-muted">Categoria de produto não encontrada.</div>
                                    ) : (
                                        <>
                                            <div className="d-flex justify-content-end gap-2 mb-4">
                                                <Link
                                                    to={`/categorias-produtos/edit/${registro.id}`}
                                            ...
                                            <h5 className="mb-3">Dados da Categoria</h5>
                                            <Row>
...
export default CategoriasProdutosViewPage
