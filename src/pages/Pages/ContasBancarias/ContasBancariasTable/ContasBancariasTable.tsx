...
import { contasBancariasList } from "interfaces/Lancamentos";
import { ContasBancariasSearch } from "interfaces/ContasBancarias";

export interface ContasBancariasTableProps {
    data: PaginateInterface<contasBancariasList> | undefined,
    getData: (data: PaginateSearch & ContasBancariasSearch) => void,
...
export const ContasBancariasTable = ({ data, getData, setPerPage, setPage, perPage, filters }: ContasBancariasTableProps) => {
    ...
    const handleRemoteDelete = async (id: number) => {
        try {
            const response = await contasBancariasService.deleteContasBancarias(id);
            if (data) {
    ...
    const handleThisRoute = async (url: string) => {
        try {
            ...
            await getData({
                ...
                contas_bancaria_id: new_url.searchParams.get('contas_bancaria_id'),
                banco_id: new_url.searchParams.get('banco_id'),
                conta_pj: Boolean(new_url.searchParams.get('conta_pj')),
                apelido: new_url.searchParams.get('apelido'),
                saldo: new_url.searchParams.get('saldo'),
                                                        ...
                                                        <table className="table align-middle table-nowrap table-striped-columns mb-0 text-center">
                                                            ...
                                                            <tbody>
                                                                {data && data.data.map((row: any, index: number) => {
                                                                    ...
                                                                    return (    
                                                                        <tr>
                                                                            <td className="text-left">{`${row.banco_codigo} - ${row.banco_nome}`}</td>
                                                                            <td>{row.apelido}</td>
                                                                            <td><span className={`text-center badge bg-${row.ativo ? 'success' : 'danger'}`}>{`${row.ativo ? 'Ativo' : 'Inativo'}`}</span></td>
                                                                            <td>{numberFormat(parseFloat(row.saldo), 2, ',', '.')}</td>
                                                                            <td>{numberFormat(parseFloat(row.saldo), 2, ',', '.')}</td>
                                                                            <td><span className={`text-center badge bg-${row.conta_pj ? 'info' : 'dark'}`}>{`${row.conta_pj ? 'PF' : 'PJ'}`}</span></td>
                                                                            <td>
                                                                                <ButtonGroup>
                                                                                    <UncontrolledDropdown direction="down">
                                                                                        ...
                                                                                        <DropdownMenu style={{ zIndex: '999' }}>
                                                                                            <Link to={`/contas-bancarias/edit/${row.id}`} state={{ source: row }}>
