...
import { buscarMesPorNumero, formatTelefone, mesesSelect, numberFormat, useNavegacao } from 'helpers/functions_helpers';
...
export interface ClientesTableProps {
    data: PaginateInterface<ClientesList> | undefined,
    getData: (data: PaginateSearch & ClientesSearch) => void,
...
export const ClientesTable = ({ data, getData, setPerPage, setPage, perPage, filters }: ClientesTableProps) => {
    ...
    const handleThisRoute = async (url: string) => {
        try {
            ...
            await getData({
                ...
                cliente_id: new_url.searchParams.get('clientes_id'),
                                                                ...
                                                                {data && data.data.map((row: any, index: number) => {
                                                                    ...
                                                                    return (    
                                                                        <tr>
                                                                            ...
                                                                            <td className="text-right">{`${formatTelefone(row.telefone)}`}</td>
                                                                            <td>
                                                                                <ButtonGroup>
                                                                                    <UncontrolledDropdown direction="down">
                                                                                        ...
                                                                                        <DropdownMenu style={{ zIndex: '999' }}>
                                                                                            <Link to={`/clientes/edit/${row.id}`} state={{ source: row }}>
