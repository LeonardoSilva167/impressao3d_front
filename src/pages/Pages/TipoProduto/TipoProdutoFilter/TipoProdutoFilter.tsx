...
import { TipoProdutoModel, TipoProdutoSearch } from "interfaces/TipoProduto/TipoProdutoInterface";


export interface TipoProdutoFilterProps {
    getRemoteTipoProdutoList: (data: any) => void
}

const TipoProdutoFilter = ({ getRemoteTipoProdutoList }: TipoProdutoFilterProps) => {
    ...
    const { handleSubmit, control, setValue, register, watch } = useForm<TipoProdutoSearch>({ defaultValues: {} })
    ...
    const getListTipoProduto = async (inputValue: string): Promise<SelectOptions[]> => {
        const listLinha = await tipoProdutoService.AsyncListTipoProduto({ palavra_chave: inputValue })
        let opt = new Array
        opt.push({ value: '', label: 'Selecione' })

        listLinha && listLinha.map((item: TipoProdutoModel) => {
                ...
                <Col xs={12}>
                    <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                        <div className='d-sm-flex align-items-center justify-content-between'>
                            ...
                            <h4 className="mb-sm-0 ms-3">Tipo Produto</h4>
                        </div>
                        <Breadcrumb pageTitle="" listClassName='mb-sm-0 pt-1 py-2'>
                            <BreadcrumbItem> <Link to="/dashboard"> <i className="ri-home-5-fill"></i> </Link> </BreadcrumbItem>
                            <BreadcrumbItem active>Tipo Produto </BreadcrumbItem>
                        </Breadcrumb>
                    </div>
                </Col>
            </Row>
                        ...
                        <CardHeader>
                            <div className="gap-2 flex-wrap">
                                <Row>
                                    ...
                                    {!showFilter &&
                                        <Col md={8}>
                                            <form onSubmit={handleSubmit(getRemoteTipoProdutoList)} >
                            ...
                            <Row>
                                <Col>
                                    <Collapse isOpen={showFilter} id="multiCollapseExample2" className="multi-collapse mt-3">
                                        <div className="">
                                            <form className="px-0 my-0 m-2" id="form-search" name="form-search" onSubmit={handleSubmit(getRemoteTipoProdutoList)}>
                                                <Row>
                                                    <Col md={5}>
                                                        <div className="mb-3">
                                                            <Label htmlFor="descricao" className="form-label">Produto</Label>
                                                            <AsyncSelectListControlled<TipoProdutoSearch>
                                                                callback={getListTipoProduto}
                                                                field={"descricao"}
                                                                control={control}
                                                                className="w-100"
                                                            />
                                                ...
                                                <Row className="mt-5">
                                                    <div className="d-flex flex-row justify-content-end align-items-center" >
                                                        ...
                                                        <Col md={6}>
                                                            <div className="ms-4 ">
                                                                <InputTextControlled<TipoProdutoSearch> field={"palavra_chave"} control={control} placeholder="Buscar..." />
                                                            </div>
                                                        </Col>
...
export default TipoProdutoFilter;     