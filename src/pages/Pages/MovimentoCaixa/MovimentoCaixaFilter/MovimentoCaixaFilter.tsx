...
import { MovimentoCaixaModel, MovimentoCaixaSearch } from "interfaces/MovimentoCaixa";


export interface MovimentoCaixaFilterProps {
    getRemoteMovimentoCaixaList: (data: any) => void
}

const MovimentoCaixaFilter = ({ getRemoteMovimentoCaixaList }: MovimentoCaixaFilterProps) => {
    const movimentoCaixaService = new MovimentoCaixaService()

    const { handleSubmit, control, setValue, register, watch } = useForm<MovimentoCaixaSearch>({ defaultValues: {} })
    ...
    const getListMovimentoCaixa = async (inputValue: string): Promise<SelectOptions[]> => {
        const listMovimentoCaixa = await movimentoCaixaService.AsyncListMovimentoCaixa({ palavra_chave: inputValue })
        let opt = new Array
        opt.push({ value: '', label: 'Selecione' })

        listMovimentoCaixa && listMovimentoCaixa.map((item: MovimentoCaixaModel) => {
                ...
                <Col xs={12}>
                    <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                        <div className="d-flex align-items-center">
                            ...
                            <h4 className="mb-0">Movimentações de Caixa</h4>
                        </div>
                        <Breadcrumb pageTitle="" listClassName='mb-sm-0 pt-1 py-2'>
                            <BreadcrumbItem> <Link to="/dashboard"> <i className="ri-home-5-fill"></i> </Link> </BreadcrumbItem>
                            <BreadcrumbItem active> Movimentações de Caixa </BreadcrumbItem>
                        </Breadcrumb>
                    </div>
                </Col>
            </Row>
                        ...
                        <CardHeader>
                            <div className="gap-2 flex-wrap">
                                <Row>
                                    ...
                                    {!showFilter &&
                                        <Col md={8}>
                                            <form onSubmit={handleSubmit(getRemoteMovimentoCaixaList)} >
                            ...
                            <Row>
                                <Col>
                                    <Collapse isOpen={showFilter} id="multiCollapseExample2" className="multi-collapse mt-3">
                                        <div className="">
                                            <form className="px-0 my-0 m-2" id="form-search" name="form-search" onSubmit={handleSubmit(getRemoteMovimentoCaixaList)}>
                                                <Row>
                                                    <Col md={5}>
                                                        <Label htmlFor="cliente_id" className="form-label">MovimentoCaixa</Label>
                                                        <AsyncSelectListControlled<MovimentoCaixaSearch>
                                                            callback={getListMovimentoCaixa}
                                                            field={"cliente_id"}
                                                            control={control}
                                                            className="w-100"
                                                        />
                                                    ...
                                                    <Col md={2}>
                                                        <div className="mb-3">
                                                            ...
                                                            <SelectListControlled<MovimentoCaixaSearch> 
                                                                options={optUF} 
                                                                field={"uf"} 
                                                                control={control} 
                                                            />
                                                    ...
                                                    <Col md={2}>
                                                        <div className="ms-4 ">
                                                            ...
                                                            <InputTextControlled<MovimentoCaixaSearch> 
                                                                field={"cidade"} 
                                                                control={control} 
                                                            />
                                                        </div>                                     
                                                ...
                                                <Row className="mt-5">
                                                    <div className="d-flex flex-row justify-content-end align-items-center" >
                                                        ...
                                                        <Col md={6}>
                                                            <div className="ms-4 ">
                                                                <InputTextControlled<MovimentoCaixaSearch> field={"palavra_chave"} control={control} placeholder="Buscar..." />
                                                            </div>
                                                        </Col>
...
export default MovimentoCaixaFilter;     