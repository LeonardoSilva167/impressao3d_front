...
export interface ItensTableProps {
    ...
    getData: (data: PaginateSearch & ItensSearch) => void
...
export const ItensTable = ({ data, getData, setPerPage, perPage }: ItensTableProps) => {
    ...
    const handleRemoteDelete = async (id: number) => {
        try {
            await itensService.deleteItens(id)
    ...
    const handleThisRoute = async (url: string) => {
        try {
            ...
            await getData({
                ...
                id_categoria_item: new_url.searchParams.get('id_categoria_item'),
                codigo: new_url.searchParams.get('codigo'),
                descricao: new_url.searchParams.get('descricao'),
                unidade_medida: new_url.searchParams.get('unidade_medida'),
            })
        } catch (error) {
            console.error(error)
        }
                                                        ...
                                                        <tbody>
                                                            {data.data.map((row: any, index: number) => (
                                                                <tr key={row.id || index}>
                                                                    ...
                                                                    <td>{formatBoolean(row.gera_custo)}</td>
                                                                    <td>{formatBoolean(row.ativo)}</td>
                                                                    <td>
                                                                        <ButtonGroup>
                                                                            <UncontrolledDropdown direction="down">
                                                                                ...
                                                                                <DropdownMenu style={{ zIndex: '999' }}>
                                                                                    <Link to={`/itens/edit/${row.id}`} state={{ source: row }}>
