...
import { DespesasList, DespesasSearch } from "interfaces/Despesas";
import CustomModal from "Components/ComponentController/Modal/CustomModal";
import { DespesasService } from "services/DespesasService";

export interface DespesasTableProps {
    data: PaginateInterface<DespesasList> | undefined,
    getData: (data: PaginateSearch & DespesasSearch) => void,
...
export const DespesasTable = ({ data, getData, setPerPage, setPage, perPage, filters }: DespesasTableProps) => {
    ...
    const handleRemoteDelete = async (id: number) => {
        try {
            const response = await despesasService.deleteDespesas(id);
            if (data) {
    ...
    const handleThisRoute = async (url: string) => {
        try {
            ...
            await getData({
                ...
                qtd_parcela: new_url.searchParams.get('qtd_parcela'),
                data_vencimento: new_url.searchParams.get('data_vencimento'),
                recorrencia: new_url.searchParams.get('recorrencia'),
                                                        ...
                                                        <table className="table align-middle table-nowrap table-striped-columns mb-0 text-center">
                                                            ...
                                                            <tbody>
                                                                {data && data.data.map((row: any, index: number) => {
                                                                    ...
                                                                    switch (row.recorrencia_status_id) {
                                                                        case 1:
                                                                            vencimento = row.data_vencimento_format;
                                                                            break;
                                                                            case 2:
                                                                        case 3:
                                                                            vencimento = row.dia_vencimento;
                                                                            break;
                                                                            case 4:
                                                                                vencimento = `${row.dia_vencimento}/${buscarMesPorNumero(row.mes_vencimento)}`;
                                                                    ...
                                                                    return (    
                                                                        <tr>
                                                                            <td>{row.nome}</td>
                                                                            {/* <td>{numberFormat(row.valor, 2, '.', '')}</td> */}
                                                                            <td>{numberFormat(parseFloat(row.valor_parcela), 2, ',', '.')}</td>
                                                                            <td>{row.recorrencia}</td>
                                                                            <td >{vencimento}</td>
                                                                            <td>{row.recorrencia_status_id == 2? row.qtd_parcela : ''}</td>
                                                                            <td><span className={`text-center badge bg-${row.ativo ? 'success' : 'danger'}`}>{`${row.ativo ? 'Ativo' : 'Inativo'}`}</span></td>
                                                                            <td>
                                                                                <ButtonGroup>
                                                                                    <UncontrolledDropdown direction="down">
                                                                                        ...
                                                                                        <DropdownMenu style={{ zIndex: '999' }}>
                                                                                            <Link to={`/despesas/edit/${row.id}`} state={{ source: row }}>
                                                                                            ...
                                                                                            <CustomModal
                                                                                                ...
                                                                                                delete={true}
                                                                                                body=""
                                                                                                onConfirmDelete={() => handleRemoteDelete(selectedId!)} // Passa a função para o modal
                                                                                            />
                                                                                        </DropdownMenu>
